...
Broad wrapper for Quantum ESPRESSO executables with grouped capabilities,
binary discovery, dry-run previews, and multi-step workflow execution.
"""
...
class QERunner:
    ...
    def run_group(self, group: str, input_map: Dict[str, str], output_dir: str = ".", cwd: Optional[str] = None) -> List[QERunResult]:
        """Run every binary in a capability group using an alias->input mapping.

        Only aliases present in ``input_map`` are executed.
        """
        if group not in QE_BINARY_GROUPS:
            raise ValueError(f"Unknown QE group: {group}")

        steps = []
        for alias in QE_BINARY_GROUPS[group]:
            if alias in input_map:
                steps.append(
                    {
                        "binary": alias,
                        "input_file": input_map[alias],
                        "output_file": str(Path(output_dir) / f"{alias}.out"),
                    }
                )
        return self.run_many(steps=steps, cwd=cwd)
    ...
    def run_many(self, steps: Sequence[Dict[str, object]], cwd: Optional[str] = None) -> List[QERunResult]:
